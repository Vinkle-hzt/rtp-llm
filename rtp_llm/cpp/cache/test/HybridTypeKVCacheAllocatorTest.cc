...
TEST_F(HybridTypeKVCacheAllocatorTest, IncrMallocRollbackFreesBackfilledExistingNullSlot) {
    auto config      = makeTinyHybridConfig();
    config.block_num = 5;  // free=4 after reserved null block
    auto allocator   = std::make_shared<HybridTypeKVCacheAllocator>(config, AllocationType::DEVICE);
    ASSERT_TRUE(allocator->init());
    ...
    auto seed_blocks = block_pool->malloc(3);
    ASSERT_EQ(seed_blocks.size(), 3u);
    ASSERT_EQ(block_pool->freeBlocksNum(), 1u);
    ...
    const auto& linear_out = batch_res->blocks(0, gid_linear);
    ASSERT_EQ(linear_out.size(), 3u);
    EXPECT_EQ(linear_out[0], seed_blocks[0]);
    EXPECT_TRUE(isNullBlockIdx(linear_out[1]));
    EXPECT_EQ(linear_out[2], seed_blocks[1]);

    const auto& full_out = batch_res->blocks(0, gid_full);
    ASSERT_EQ(full_out.size(), 1u);
    EXPECT_EQ(full_out[0], seed_blocks[2]);
