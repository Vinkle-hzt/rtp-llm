...
void CudaGraphRunner::prepareAttentionInputs(const PyModelInputs& inputs,
    ...
    // or from the engine main thread (PyWrappedModel::forward), neither of which holds
    // the GIL on entry. pybind11's attr() and call operator construct
...
void CudaGraphRunner::initCapture() {
    ...
    if (enable_cuda_graph_) {
        ...
        if (is_prefill_cuda_graph_mode_) {
            ...
            const int post_check_tokens = max_num_token_;
...
void CudaGraphRunner::prepareCaptureInputs(PyModelInputs& inputs, int batch_size, int seq_len_or_tokens) {
    ...
    inputs.attention_inputs.is_prefill       = is_prefill_cuda_graph_mode_ || num_tokens_per_bs_ > 1;
    inputs.attention_inputs.is_target_verify = is_target_verify_;
    // Draft prefill cudagraph mode uses one graph per concrete draft batch count.
    // Keep the token slice equal to batch_size * num_tokens_per_bs_ so Python
    // forward and FlashInfer metadata agree on the number of Q tokens.
