...
def _build_prefill_inputs(
    ...
    attention_inputs.sequence_lengths_host = torch.empty(0, dtype=torch.int32)
    attention_inputs.sequence_lengths_device = torch.empty(
        0, dtype=torch.int32, device="cuda"
    )
    attention_inputs.max_sequence_length = seq_len
