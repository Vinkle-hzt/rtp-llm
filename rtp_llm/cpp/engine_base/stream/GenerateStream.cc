...
void GenerateStream::specUpdate(const StreamSpecUpdateInfo& update_info) {
    // Plan N5 contract: this function is the worker-thread entry point for
    ...
    // RTP_LLM_MTP_ASYNC_STOP_EXTRA env gate (§6.1 "one-extra" budget) is
    ...
    // accept-then-clip pipeline (§6.0) handles boundary correctness.
    RTP_LLM_PROFILE_FUNCTION();
    std::lock_guard<std::mutex> lock(*mutex_);
    ...
    *is_context_stream_ = false;
    ...
    // Phase 3.1: cache the per-stream GPU propose tokens for the next decode step.
