...
void tpSyncModelInputs(GptModelInputs& inputs, const ParallelismConfig& parallelism_config) {
    ...
    {
        // Phase 3.1: encode root-side tensor device for fields that may live on
    ...
    if (is_non_root) {
        ...
        // Phase 3.1: respect the root-side device bitmap so GPU-resident fields
    ...
    if (!is_root) {
        ...
        if (gpu_total_bytes > 0) {
            ...
            for (auto& e : gpu_entries) {
                ...
                flush_fused_copy();
                auto src_tensor =
                    torch::from_blob(packed_base + e.offset, e.tensor->sizes(), e.tensor->options());
