...
struct TensorHolder {
    ...
    template<typename T,
             typename = std::enable_if_t<
                 !std::is_same_v<std::decay_t<T>, torch::Tensor> && !std::is_same_v<std::decay_t<T>, TensorHolder>>>
    void hold(const T& object) {
        objects.emplace_back(std::make_unique<HeldObject<std::decay_t<T>>>(object));
    }

    void release() {
        // Keep two released rounds alive so tensors used by asynchronous
        // copies or kernels cannot be recycled before the queued work ends.
