...
#pragma unroll
    ...
    if (threadIdx.x < 64) {
        float warp_exclusive_prefix_sum = (threadIdx.x < BLOCK_THREADS / 64) ? smem_prefix_sum[threadIdx.x] : 0;
...
#pragma unroll
    ...
    bool greater_than_u_diff[VEC_SIZE];
#if defined(FLASHINFER_CUB_SUBTRACTLEFT_DEFINED)
    // ROCm 7.x hipCUB/rocPRIM uses SubtractLeft instead of FlagHeads
    BlockAdjacentDifference<bool, BLOCK_THREADS>(temp_storage->block_prim.adj_diff)
...
__global__ void ChainSpeculativeSampling(DType*   draft_probs,
    ...
    if (s_pos == num_speculative_tokens) {
        return;
    }
    uint32_t pos = s_pos;
...
#pragma unroll
    ...
    if (tx == 0) {
        ...
        output_token_ids[row_idx * (num_speculative_tokens + 1) + pos] = sampled_id;
        // pad remaining tokens with -1
        for (uint32_t p = pos + 1; p < num_speculative_tokens + 1; ++p) {
            output_token_ids[row_idx * (num_speculative_tokens + 1) + p] = -1;
        }
    }
}
...
hipError_t ChainSpeculativeSampling(DType*      draft_probs,
                                    IdType*     draft_token_ids,
                                    DType*      uniform_samples,
                                    DType*      target_probs,
                                    IdType*     output_token_ids,
    ...
    DISPATCH_ALIGNED_VEC_SIZE(
        vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
            ChainSpeculativeSampling<BLOCK_THREADS, SCAN_ALGO, REDUCE_ALGO, VEC_SIZE, DETERMINISTIC, DType, IdType>
...
void chain_speculative_sampling(at::Tensor draft_probs,
    ...
    hipError_t  status =
        ChainSpeculativeSampling<float, int>(static_cast<float*>(draft_probs.data_ptr()),
                                             static_cast<int*>(draft_token_ids.data_ptr()),
                                             ...
                                             static_cast<int*>(output_emitted_draft_token_num.data_ptr()),
                                             batch_size,
