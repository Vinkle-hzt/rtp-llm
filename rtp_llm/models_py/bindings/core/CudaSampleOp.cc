...
void mappingDraft2TargetCpuTyped(IdType*                          tokens,
                                 const int64_t*                   d2t_map,
                                 const MappingDraft2TargetParams& params,
                                 int                              d2t_map_size) {
    for (int i = 0; i < params.batch_size; i++) {
        for (int j = params.token_offset; j < params.token_stride; j++) {
            int     idx      = i * params.token_stride + j;
            int64_t token_id = static_cast<int64_t>(tokens[idx]);
            if (token_id >= 0 && token_id < d2t_map_size) {
                tokens[idx] = static_cast<IdType>(d2t_map[token_id]);
            }
        }
    }
}

void mappingDraft2TargetCpu(const MappingDraft2TargetParams& params, int d2t_map_size) {
    const int64_t* d2t_map_ptr = params.d2t_map.data_ptr<int64_t>();
    if (params.tokens.dtype() == torch::kInt32) {
        mappingDraft2TargetCpuTyped(params.tokens.data_ptr<int32_t>(), d2t_map_ptr, params, d2t_map_size);
    } else if (params.tokens.dtype() == torch::kInt64) {
        mappingDraft2TargetCpuTyped(params.tokens.data_ptr<int64_t>(), d2t_map_ptr, params, d2t_map_size);
    } else {
        RTP_LLM_CHECK_WITH_INFO(false, "Unsupported token dtype for mappingDraft2Target");
    }
}

template<typename DeviceMapper>
void mappingDraft2TargetDevice(const MappingDraft2TargetParams& params,
                               int                              d2t_map_size,
                               const DeviceMapper&              map_device) {
    if (params.tokens.dtype() == torch::kInt32) {
        map_device(params.tokens.data_ptr<int32_t>(), params, d2t_map_size);
    } else if (params.tokens.dtype() == torch::kInt64) {
        map_device(params.tokens.data_ptr<int64_t>(), params, d2t_map_size);
    } else {
        RTP_LLM_CHECK_WITH_INFO(false, "Unsupported token dtype for mappingDraft2Target");
    }
}
...
template<typename DeviceMapper>
void mappingDraft2TargetImpl(const MappingDraft2TargetParams& params, const DeviceMapper& map_device) {
    ...
    if (!params.tokens.is_cuda() && !params.d2t_map.is_cuda()) {
        mappingDraft2TargetCpu(params, d2t_map_size);
    } else if (params.tokens.is_cuda() && params.d2t_map.is_cuda()) {
        mappingDraft2TargetDevice(params, d2t_map_size, map_device);
    } else {
...
void mappingDraft2Target(const MappingDraft2TargetParams& params) {
    auto stream = at::cuda::getCurrentCUDAStream().stream();
    mappingDraft2TargetImpl(params, [stream](auto* tokens, const MappingDraft2TargetParams& params, int d2t_map_size) {
        invokeMappingDraft2Target(tokens,
                                  params.batch_size,
                                  params.token_offset,
                                  params.token_stride,
