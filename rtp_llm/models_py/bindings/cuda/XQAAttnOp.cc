...
ParamsBasePtr XQAAttnOp::prepare(torch_ext::PyAttentionInputs attn_inputs) {
    ...
    int          batch_size = attn_inputs.sequence_lengths_device.size(0);
    ...
    params->max_seq_len      = std::max(0, attn_inputs.max_sequence_length - 1);
    params->sequence_lengths = attn_inputs.sequence_lengths_device;
