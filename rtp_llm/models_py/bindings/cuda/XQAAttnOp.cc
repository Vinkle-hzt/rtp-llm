...
void updateKvCacheOffsetTensor(const torch::Tensor& kv_cache_offset, const torch::Tensor& block_ids) {
    ...
    RTP_LLM_CHECK_WITH_INFO(kv_cache_offset.scalar_type() == torch::kInt32,
                            "XQAAttnOp expects int32 kv_cache_offset");
    RTP_LLM_CHECK_WITH_INFO(kv_cache_offset.dim() == 4 && kv_cache_offset.size(1) == 1
                                && kv_cache_offset.size(2) == 2,
