...
class TRTLLMFMHAv2PagedPrefillOp:
    ...
    def prepare(self, attn_inputs: PyAttentionInputs) -> TRTLLMFMHAv2Params:
        ...
        if attn_inputs.disable_flash_infer:
            ...
            max_q_len = max(
                attn_inputs.total_tokens // max(1, batch_size),
                _TRTLLM_FMHA_V2_MIN_SEQ_LEN,
            )
            max_kv_len = max(
                (
                    attn_inputs.max_sequence_length
                    if attn_inputs.is_cuda_graph
                    else attn_inputs.max_sequence_length
                    + attn_inputs.total_tokens // max(1, batch_size)
                    - 1
                ),
                _TRTLLM_FMHA_V2_MIN_SEQ_LEN,
...
class TRTLLMFMHAv2PrefillOp:
    ...
    def support(
        ...
        # MTP target verify and draft prefill both reuse paged KV cache. Keep
        # their device-only metadata away from this non-paged implementation.
        if attn_inputs.disable_flash_infer:
            return False
        has_prefix = (
...
class FlashInferTRTLLMFMHAv2PagedPrefillImpl(FMHAImplBase):
    ...
    def __init__(
        ...
        self.use_device_cache_plan = attn_inputs.disable_flash_infer
        if self.use_device_cache_plan:
            self.attn_configs = attn_configs
            if attn_configs.rope_config.style == RopeStyle.No:
                self.rope_impl = None
            elif attn_configs.rope_config.style == RopeStyle.Mrope:
                self.rope_impl = MropeRotaryEmbeddingOp(attn_configs)
            else:
            ...
            self._prepare_device_cache_plan(attn_inputs, forbid_realloc=False)
            if isinstance(self.rope_impl, MhaRotaryEmbeddingOp):
    ...
    def _split_qkv(
        ...
        q_size = self.attn_configs.head_num * head_dim
        kv_size = self.attn_configs.kv_head_num * head_dim
        q, k, v = torch.split(qkv, [q_size, kv_size, kv_size], dim=-1)
        return (
            q.reshape(q.shape[0], self.attn_configs.head_num, head_dim),
            k.reshape(k.shape[0], self.attn_configs.kv_head_num, head_dim),
            v.reshape(v.shape[0], self.attn_configs.kv_head_num, head_dim),
        )
    ...
    def forward(
        ...
        if self.use_device_cache_plan:
            if self.rope_impl is not None:
                if isinstance(self.rope_impl, MropeRotaryEmbeddingOp):
                    query, key, value = self.rope_impl.forward(
                        qkv,
                        self.attn_inputs.combo_position_ids,
                        self.rope_params.positions_d,
                    )
                else:
    ...
    def prepare_cuda_graph(self, attn_inputs: PyAttentionInputs) -> None:
        ...
        if self.use_device_cache_plan:
            self._prepare_device_cache_plan(attn_inputs, forbid_realloc=True)
            return
